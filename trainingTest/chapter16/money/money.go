...
func (m Money) Times(multiplier int) Expression {
	return CreateMoney(m.amount*multiplier, m.currencyCode)
}

// Puls 通貨の足し算
func (m Money) Puls(addend Expression) Expression {
	return Sum{m, addend}
}
...
func (m Money) Reduce(b *Bank, to string) Money {
	var rate int = b.Rate(m.currencyCode, to)
	return CreateMoney(m.amount/rate, to)
}
